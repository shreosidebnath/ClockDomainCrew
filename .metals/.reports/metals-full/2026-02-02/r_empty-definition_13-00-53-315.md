error id: file:///C:/Users/benja/ethernet-mac-chisel/src/main/scala/ethernet/xgmii2axis.scala:IO.
file:///C:/Users/benja/ethernet-mac-chisel/src/main/scala/ethernet/xgmii2axis.scala
...
non-local guesses:
	 ...
	 -scala/Predef.IO().
offset: 181
uri: file:///C:/Users/benja/ethernet-mac-chisel/src/main/scala/ethernet/xgmii2axis.scala
text:
```scala
package ethernet
import chisel3._
...
class XGMII2AXIS extends Module {
  ...
  // Create an instance of EthMacDefinitions to access everything
  val macDefs = Module(new EthMacDefinitions)
  
  // Or use the values directly if they're in a companion object
  // val XGMII_START = EthMacDefinitions.XGMII_START
  // val crc8B = EthMacDefinitions.EthernetCrc.crc8B
  
  // Access constants
  val XGMII_START = macDefs.S  // Your class uses S for START
  val XGMII_TERM = macDefs.T   // Your class uses T for TERM
  val XGMII_ERROR = macDefs.E  // Your class uses E for ERROR
  val XGMII_IDLE = macDefs.I   // Your class uses I for IDLE
  val CRC802_3_PRESET = macDefs.crc8023Preset
  
  // Access helper functions
  def sof_lane0(d: UInt, c: UInt): Bool = macDefs.SofLane0(d, c)
  def sof_lane4(d: UInt, c: UInt): Bool = macDefs.SofLane4(d, c)
  def is_tchar(data: UInt): Bool = macDefs.IsTchar(data)
  
  // Access CRC functions
  ...
  def crc_rev(crc: UInt): UInt = macDefs.CrcRev(crc)
  ...
  when(io.rst) {
    // Reset all adapter registers
    tvalid_i := false.B
    fsm := State.SRES
    rx_statistics_valid_reg := false.B
    rx_statistics_vector_reg := 0.U
    
    // Initialize all registers (autoreset equivalent)
    aux_dw := 0.U
    bad_frames_reg := 0.U
    c_reg := 0.U
    calcted_crc := 0.U
    ...
    // Update statistics based on completed frames
    when(tvalid_reg && tlast_reg && tuser_reg(0) === 0.U) {
      good_frames_reg := good_frames_reg + 1.U
    }
    
    when(tvalid_reg && tlast_reg && tuser_reg(0) === 1.U) {
      ...
      is(State.ST_LANE0) {
        ...
        switch(io.xgmii_c) {
          ...
          is("hFF".U(8.W)) {
            ...
            tkeep_i := "h0F".U(8.W)  // Only first 4 bytes valid
            tvalid_d0 := false.B
            tlast_i := true.B
