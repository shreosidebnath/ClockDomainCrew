...
#ifndef VERILATED_VAXIS_LOOPBACK_H_
#define VERILATED_VAXIS_LOOPBACK_H_  // guard
...
class alignas(VL_CACHE_LINE_BYTES) Vaxis_loopback VL_NOT_FINAL : public VerilatedModel {
  ...
  public:
    ...
    Vaxis_loopback___024root* const rootp;
    ...
    explicit Vaxis_loopback(VerilatedContext* contextp, const char* name = "TOP");
    explicit Vaxis_loopback(const char* name = "TOP");
    /// Destroy the model; called (often implicitly) by application code
    virtual ~Vaxis_loopback();
  private:
    VL_UNCOPYABLE(Vaxis_loopback);  ///< Copying not allowed
