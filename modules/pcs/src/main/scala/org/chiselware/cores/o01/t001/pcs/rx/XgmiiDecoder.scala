...
class XgmiiDecoder(
  ...
  val io = IO(new Bundle {
    ...
    val encoded_rx_data_valid = Input(
      Bool()
    ) // Default true in SV, must be driven in Chisel
  ...
  if (dataW == 64) {
    ...
    encodedRxDataValidInt :=
      encodedRxDataValidReg && Mux(gbxIfEn.B, io.encoded_rx_data_valid, true.B)
  ...
  when(!encodedRxDataValidInt) {
    ...
    switch(blockTypeNibble) {
      ...
      is(BLOCK_TYPE_OS_4(7, 4)) {
        ...
        val rxd31_0 =
          decodedCtrl.take(4).reverse // Get elements 0-3, reverse for Cat
        val rxdLow = Cat(rxd31_0)
        val rxdHigh = encodedRxDataInt(63, 40)

        val osByte = Mux(
          encodedRxDataInt(39, 36) === O_SEQ_OS,
          XGMII_SEQ_OS,
          XGMII_ERROR
        )
        ...
        xgmiiRxcNext :=
          0x1f.U(8.W) // 4'h1 (upper) ## 4'hf (lower) -> 0001 1111 -> 0x1f
      ...
      is(BLOCK_TYPE_OS_START(7, 4)) {
        ...
        // Lower Part
        val osByteLow = Mux(
          encodedRxDataInt(35, 32) === O_SEQ_OS,
          XGMII_SEQ_OS,
          XGMII_ERROR
        )
      ...
      is(BLOCK_TYPE_OS_0(7, 4)) {
        ...
        val lowBad = encodedRxDataInt(35, 32) =/= O_SEQ_OS
        val upperErr =
          Cat(decodeErr(7), decodeErr(6), decodeErr(5), decodeErr(4)) =/= 0.U
