...
class PcsTxInterface(
  ...
  when(prbs31En.B && io.cfg_tx_prbs31_enable) {
    ...
    serdes_tx_data_reg :=
      Mux(scramblerDisable.B, io.encoded_tx_data, scrambler.io.data_out)
    serdes_tx_hdr_reg := io.encoded_tx_hdr
  }

  val tx_data_int =
    if (bitReverse)
      Reverse(serdes_tx_data_reg)
    else
      serdes_tx_data_reg
  val tx_hdr_int =
    if (bitReverse)
      Reverse(serdes_tx_hdr_reg)
    else
      serdes_tx_hdr_reg

  io.serdes_tx_data := ShiftRegister(tx_data_int, serdesPipeline)
  io.serdes_tx_hdr := ShiftRegister(tx_hdr_int, serdesPipeline)
  io.serdes_tx_data_valid :=
    ShiftRegister(RegNext(io.encoded_tx_data_valid), serdesPipeline)
  io.serdes_tx_hdr_valid :=
    ShiftRegister(RegNext(io.encoded_tx_hdr_valid), serdesPipeline)
  io.serdes_tx_gbx_sync :=
    ShiftRegister(RegNext(io.tx_gbx_sync), serdesPipeline)
}
