...
class XgmiiEncoder(
  ...
  for (i <- 0 until ctrlWInt) {
    when (xgmii_txc_bits(i)) {
      ...
      val known_codes = Seq(
        XGMII_IDLE, XGMII_LPI, XGMII_ERROR, 
        XGMII_RES_0, XGMII_RES_1, XGMII_RES_2, XGMII_RES_3, XGMII_RES_4, XGMII_RES_5
      )
      val is_known = known_codes.map(c => xgmii_txd_bytes(i) === c).reduce(_ || _)
      when (!is_known) {
        encoded_ctrl_vec(i) := CTRL_ERROR
        encode_err_vec(i) := true.B
      }
  ...
  def ctrlSlice(high: Int, low: Int): UInt = {
  ...
  when (xgmii_tx_valid_int) {
    ...
    when (xgmii_txc_int === 0.U) {
      ...
      when (c === 0x1f.U && d(4) === XGMII_SEQ_OS) {
        encoded_tx_data_next := Cat(d(7), d(6), d(5), O_SEQ_OS, ctrlSlice(3, 0), BLOCK_TYPE_OS_4)
        tx_bad_block_next := encode_err_vec(0) || encode_err_vec(1) || encode_err_vec(2) || encode_err_vec(3)
      } .elsewhen (c === 0x1f.U && d(4) === XGMII_START) {
        encoded_tx_data_next := Cat(d(7), d(6), d(5), 0.U(4.W), ctrlSlice(3, 0), BLOCK_TYPE_START_4)
        tx_bad_block_next := encode_err_vec(0) || encode_err_vec(1) || encode_err_vec(2) || encode_err_vec(3)
      } .elsewhen (c === 0x11.U && d(0) === XGMII_SEQ_OS && d(4) === XGMII_START) {
        encoded_tx_data_next := Cat(d(7), d(6), d(5), 0.U(4.W), O_SEQ_OS, d(3), d(2), d(1), BLOCK_TYPE_OS_START)
        tx_bad_block_next := false.B
      } .elsewhen (c === 0x11.U && d(0) === XGMII_SEQ_OS && d(4) === XGMII_SEQ_OS) {
        encoded_tx_data_next := Cat(d(7), d(6), d(5), O_SEQ_OS, O_SEQ_OS, d(3), d(2), d(1), BLOCK_TYPE_OS_04)
        tx_bad_block_next := false.B
        ...
        encoded_tx_data_next := Cat(ctrlSlice(7, 4), O_SEQ_OS, d(3), d(2), d(1), BLOCK_TYPE_OS_0)
        tx_bad_block_next := encode_err_vec(4) || encode_err_vec(5) || encode_err_vec(6) || encode_err_vec(7)
      } .elsewhen (c === 0xff.U && d(0) === XGMII_TERM) {
        encoded_tx_data_next := Cat(ctrlSlice(7, 1), 0.U(7.W), BLOCK_TYPE_TERM_0)
        tx_bad_block_next := encode_err_flat(7, 1) =/= 0.U
      } .elsewhen (c === 0xfe.U && d(1) === XGMII_TERM) {
        encoded_tx_data_next := Cat(ctrlSlice(7, 2), 0.U(6.W), d(0), BLOCK_TYPE_TERM_1)
        tx_bad_block_next := encode_err_flat(7, 2) =/= 0.U
      } .elsewhen (c === 0xfc.U && d(2) === XGMII_TERM) {
        encoded_tx_data_next := Cat(ctrlSlice(7, 3), 0.U(5.W), xgmii_txd_int(15, 0), BLOCK_TYPE_TERM_2)
        tx_bad_block_next := encode_err_flat(7, 3) =/= 0.U
      } .elsewhen (c === 0xf8.U && d(3) === XGMII_TERM) {
        encoded_tx_data_next := Cat(ctrlSlice(7, 4), 0.U(4.W), xgmii_txd_int(23, 0), BLOCK_TYPE_TERM_3)
        tx_bad_block_next := encode_err_flat(7, 4) =/= 0.U
      } .elsewhen (c === 0xf0.U && d(4) === XGMII_TERM) {
        encoded_tx_data_next := Cat(ctrlSlice(7, 5), 0.U(3.W), xgmii_txd_int(31, 0), BLOCK_TYPE_TERM_4)
        tx_bad_block_next := encode_err_flat(7, 5) =/= 0.U
      } .elsewhen (c === 0xe0.U && d(5) === XGMII_TERM) {
        encoded_tx_data_next := Cat(ctrlSlice(7, 6), 0.U(2.W), xgmii_txd_int(39, 0), BLOCK_TYPE_TERM_5)
        tx_bad_block_next := encode_err_flat(7, 6) =/= 0.U
      } .elsewhen (c === 0xc0.U && d(6) === XGMII_TERM) {
        encoded_tx_data_next := Cat(ctrlSlice(7, 7), 0.U(1.W), xgmii_txd_int(47, 0), BLOCK_TYPE_TERM_6)
